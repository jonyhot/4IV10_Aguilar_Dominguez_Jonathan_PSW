function initCanvas(){
    ...
    var homerImage = new Image(); // nave
    var enemiespic1 = new Image(); // enemigo 1
    var enemiespic2 = new Image(); // enemigo 2

    // Imagen de fondo y imagen de homero
    backgroundImage. src = "imagenes/fondo.jpg";  //Imagen de fondo
    homerImage.src = "imagenes/Homer_Simpson.png"; //Spaceship picture
    // Enemigos fotos
    enemiespic1.src = "imagenes/flanders.png";
    enemiespic2.src = "imagenes/burns.png"; //Enemies picture

    // Ancho y alto 
    var cW = ctx.canvas.width; // 700px 
    var cH = ctx.canvas.height;// 600px

    // molde para los enemigos
    ...
    // Reducir una o dos funciones repetitiva
    var enemies = [
    ...
    function Launcher(){
        ...
        this.render = function () {
            ...
            ctx.drawImage(homerImage,this.x,this.y, 100, 90); 
        ...
        // Detectar impacto de lanzamiento
            ...
            for (var i = 0; i < enemies.length; i++) {
                ...
                if(m.x+m.w >= e.x && 
                   ...
                   m.y <= e.y+e.h){
                    ...
                    enemies.splice(i, 1); 
                    document.querySelector('.barra').innerHTML = " rosquillazos "+ e.id+ " ";
                }
            }
        }
        ...
        this.hitDetectLowerLevel = function(enemy){
            ...
            if ((enemy.y < this.y + 25 && enemy.y > this.y - 25) &&
                (enemy.x < this.x + 45 && enemy.x > this.x - 45)) { // Comprobar la dirección del enemigo
                    this.gameStatus.over = true;
   ...
   document.addEventListener('keydown', function(event) {
    ...
    document.addEventListener('keyup', function(event){
         if(event.keyCode == 38) // felcha arriba
         {
           launcher.y -= 0;
           launcher.direccion = '';
         }
    ...
    document.addEventListener('keydown', function(event){
         if(event.keyCode == 80) // resetear juego
         {
          location.reload();
         }
    });
    ...
    // Activar la barra de espacio al hacer click
    document.addEventListener('keydown', function(event) {
        if(event.keyCode == 32) {
           launcher.misiles.push({x: launcher.x + launcher.w*.5, y: launcher.y, w: 3,h: 10});
        }
